...
# Author: https://github.com/arjun-234

class Messenger:
    """Implement a insta-messenger to parse incoming webhooks and send msgs."""
    ...
    async def message(
        ...
        """Handle an incoming event from the insta webhook."""
...
class MessengerBot(OutputChannel):
        ...
        **kwargs: Any,
        ...
        if len(buttons) > 3:
            rasa.shared.utils.io.raise_warning(
                "Instagram API currently supports a maximum of 3 buttons."
                "Any additional buttons will be ignored."
            )
            ...
            # buttons in the insta-messenger framework - so we need to create the
            # payload on our own
            payload = {
                "attachment": {
                    "type": "template",
    ...
    def _convert_to_quick_reply(quick_replies: List[Dict[Text, Any]]) -> QuickReplies:
        """Convert quick reply dictionary to insta QuickReplies object"""
...
class InstagramInput(InputChannel):
    ...
    def __init__(self, insta_verify: Text, insta_secret: Text, insta_access_token: Text) -> None:
        ...
        This channel requires several settings for proper authentication and message validation.
        For detailed setup instructions, please refer to the following guide:
        https://medium.com/@arjun.darji/rasa-and-instagram-a-chatbot-love-story-a30f764a81b4

        Args:
            insta_verify (str): Insta Verification string
                (can be chosen by yourself during webhook creation)
            insta_secret (str): Meta Developer App Secret
            insta_access_token (str): Access token for posting on behalf of the instagram page
        """
